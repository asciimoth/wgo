...
func applyProfileToDevice(dev *device.Device, profile *amnezia.Profile) error {
	privateKey, err := device.ParsePrivateKey(strings.TrimSpace(profile.Interface.PrivateKey))
	if err != nil {
		return fmt.Errorf("parse interface private key: %w", err)
	}
	if privateKey.IsZero() {
		return errors.New("profile has no interface private key")
	}
	if public := strings.TrimSpace(profile.Interface.PublicKey); public != "" && privateKey.PublicKey().Base64() != public {
		return errors.New("interface private key does not match profile public key")
	}
	...
	allowedIPs, err := parseAllowedIPs(profile.Peer.AllowedIPs)
	if err != nil {
		return err
	}
	awg, err := deviceAmneziaWGConfig(profile.AmneziaWG)
	if err != nil {
		return err
	}
	listenPort, err := uint16Value("listen port", profile.Interface.ListenPort)
	if err != nil {
		return err
	}
	peer := device.PeerConfig{
	...
	if profile.Peer.PersistentKeepalive.Set {
		if profile.Peer.PersistentKeepalive.Min == profile.Peer.PersistentKeepalive.Max {
			keepalive, err := uint16Value("persistent keepalive", int(profile.Peer.PersistentKeepalive.Min))
			if err != nil {
				return err
			}
			peer.PersistentKeepaliveInterval = keepalive
		} else {
			keepalive, err := deviceRange(profile.Peer.PersistentKeepalive)
			if err != nil {
				return fmt.Errorf("persistent keepalive: %w", err)
			}
			peer.PersistentKeepaliveRange = &keepalive
		}
	}
	return dev.ApplyConfig(device.DeviceConfig{
		PrivateKey: privateKey,
		ListenPort: listenPort,
		Fwmark:     profile.Interface.FirewallMark,
		AmneziaWG:  awg,
		Peers:      []device.PeerConfig{peer},
	}, device.ApplyConfigOptions{
		ReplacePeers:   true,
		ApplyEndpoints: true,
		ActivatePeers:  true,
	})
...
func parseAllowedIPs(values []string) ([]netip.Prefix, error) {
	...
	if len(prefixes) == 0 {
		return nil, errors.New("profile has no peer allowed IPs")
	}
	return prefixes, nil
}

func deviceAmneziaWGConfig(src amnezia.AmneziaWGConfig) (device.AmneziaWGConfig, error) {
	cfg := device.DefaultAmneziaWGConfig()
	version, err := deviceAmneziaWGVersion(src.ProtocolVersion)
	if err != nil {
	...
	if !fitsInt(src.InitPadding) || !fitsInt(src.ResponsePadding) ||
		!fitsInt(src.CookiePadding) || !fitsInt(src.TransportPadding) {
		return device.AmneziaWGConfig{}, errors.New("AmneziaWG padding exceeds int range")
	}
	...
	if strings.TrimSpace(src.HeaderProtectionKey) != "" {
		key, err := parseHeaderProtectionKey(src.HeaderProtectionKey)
		if err != nil {
			return device.AmneziaWGConfig{}, fmt.Errorf("parse header protection key: %w", err)
		}
		cfg.HeaderProtectionKey = key
	}
	if cfg.ContentPadding, err = deviceRange(src.ContentPaddingAddition); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("content padding: %w", err)
	}
	if cfg.RekeyAfterTime, err = deviceRange(src.RekeyAfterTime); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("rekey after time: %w", err)
	}
	if cfg.RekeyTimeout, err = deviceRange(src.RekeyTimeout); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("rekey timeout: %w", err)
	}
	if cfg.RejectAfterTime, err = deviceRange(src.RejectAfterTime); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("reject after time: %w", err)
	}
	if cfg.KeepaliveTimeout, err = deviceRange(src.KeepaliveTimeout); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("keepalive timeout: %w", err)
	}
	if cfg.MaxHandshakeAttempts, err = deviceRange(src.MaxHandshakeAttempts); err != nil {
		return device.AmneziaWGConfig{}, fmt.Errorf("max handshake attempts: %w", err)
	}
...
func deviceAmneziaWGVersion(value string) (device.AmneziaWGVersion, error) {
	...
	default:
		return device.AmneziaWGVersionAuto, fmt.Errorf("unsupported AmneziaWG protocol version %q", value)
	}
}

func deviceRange(src amnezia.Uint32Range) (device.AmneziaWGRange, error) {
	if !src.Set {
		return device.AmneziaWGRange{}, nil
	}
	return device.AmneziaWGRange{Min: src.Min, Max: src.Max, Set: true}, nil
}

func fitsInt(value uint32) bool {
	return uint64(value) <= uint64(int(^uint(0)>>1))
}

func parseHeaderProtectionKey(value string) (device.AmneziaWGHeaderProtectionKey, error) {
...
func uint16Value(name string, value int) (uint16, error) {
